...
def plot_confusion_matrix(cm, classes=['Background', 'Water Body']):
    ...
    plt.savefig('files/confusion_matrix.png')
    plt.clf()
...
if __name__ == "__main__":
    ...
    test_images_path = os.path.normpath(os.path.join(base_path, "test", "images"))
    test_masks_path = os.path.normpath(os.path.join(base_path, "test", "masks"))
    ...
    test_image_names = [os.path.basename(p) for p in test_image_paths]
    test_mask_names = [os.path.basename(p) for p in test_mask_paths]
    if set(test_image_names) != set(test_mask_names):
        print("WARNING: Test images and masks do not match.")
